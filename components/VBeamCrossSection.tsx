...
export const VBeamCrossSection: React.FC<VBeamCrossSectionProps> = ({ inputs, activeInput }) => {
  ...
  const geom = useMemo(() => {
    ...
    const La = 2 * Lb;
    const Lc = Math.hypot(roofWebOuterCornerR.x - apexOuter.x, roofWebOuterCornerR.y - apexOuter.y);
  ...
  // Vector pháp tuyến cho Lc (cạnh mái trái)
  const ldx = P.roofWebCornerL.x - P.apexOut.x;
  const ldy = P.roofWebCornerL.y - P.apexOut.y;
      ...
      <div className="w-full h-[620px]">
        <svg viewBox={`0 0 ${W} ${Hsvg}`} className="w-full h-full">
          ...
          <g stroke={"#94a3b8"} fill="none">
            <line x1={P.apexOut.x + lnx*25} y1={P.apexOut.y + lny*25} x2={P.roofWebCornerL.x + lnx*25} y2={P.roofWebCornerL.y + lny*25}/>
            <line x1={P.apexOut.x} y1={P.apexOut.y} x2={P.apexOut.x + lnx*25} y2={P.apexOut.y + lny*25} strokeDasharray="3 3" opacity={0.7}/>
            <line x1={P.roofWebCornerL.x} y1={P.roofWebCornerL.y} x2={P.roofWebCornerL.x + lnx*25} y2={P.roofWebCornerL.y + lny*25} strokeDasharray="3 3" opacity={0.7}/>
            <text x={(P.apexOut.x + P.roofWebCornerL.x)/2 + lnx*32} y={(P.apexOut.y + P.roofWebCornerL.y)/2 + lny*32}
                  className="text-xs font-semibold" dominantBaseline="middle" textAnchor="middle" fill={"#94a3b8"}
                  transform={`rotate(${-10} ${(P.apexOut.x + P.roofWebCornerL.x)/2 + lnx*32} ${(P.apexOut.y + P.roofWebCornerL.y)/2 + lny*32})`}>
              {`Lc = ${geom.Lc.toFixed(2)}`}
            </text>
          </g>
          ...
          <g fill="#94a3b8">
            ...
            {/* Góc alpha giữa V-web và mái */}
            <path d={`M ${P.webInR.x + S(22 * Math.cos(Math.PI/3))} ${P.webInR.y + S(22 * Math.sin(Math.PI/3))} A ${S(22)} ${S(22)} 0 0 1 ${P.webInR.x + S(22 * Math.cos(alpha))} ${P.webInR.y - S(22 * Math.sin(alpha))}`} fill="none" stroke="#94a3b8"/>
            <text x={P.webInR.x + S(26)} y={P.webInR.y + S(10)} className="text-xs font-semibold">{`α = 10°`}</text>
          </g>
        </svg>
      </div>
    </div>
