  ...
  return (
    <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
      <div className="grid grid-cols-2 gap-4 mb-4">
        <img src="./Dam V thuc te.png" alt="V beam - ảnh thực tế" className="w-full h-auto rounded"/>
        <img src="./Dam V.png" alt="V beam - ảnh có ký hiệu kích thước" className="w-full h-auto rounded"/>
      </div>
      ...
      <div className="w-full h-[620px]">
        <svg viewBox={`0 0 ${W} ${Hsvg}`} className="w-full h-full">
          ...
          />
          <Dimension
            x1={cx - S(t2 / 2)} y1={by - S(geom.yJunc) + 16}
            x2={cx + S(t2 / 2)} y2={by - S(geom.yJunc) + 16}
            label={`t2 = ${t2.toFixed(2)}`} isHighlighted={dimHL("t2")}
            onMouseEnter={() => setHover("t2")} onMouseLeave={() => setHover(null)}
          />

          {/* h3 (đo dọc V-web) */}
          <g stroke={dimHL("h3") ? "#22c55e" : "#94a3b8"} fill="none" onMouseEnter={() => setHover("h3")} onMouseLeave={() => setHover(null)}>
            <line x1={cx + S(t2/2) + nx*18} y1={by - S(geom.yJunc) + ny*18} x2={P.webInR.x + nx*18} y2={P.webInR.y + ny*18}/>
            <line x1={cx + S(t2/2)} y1={by - S(geom.yJunc)} x2={cx + S(t2/2) + nx*18} y2={by - S(geom.yJunc) + ny*18} strokeDasharray="3 3" opacity={0.7}/>
            <line x1={P.webInR.x} y1={P.webInR.y} x2={P.webInR.x + nx*18} y2={P.webInR.y + ny*18} strokeDasharray="3 3" opacity={0.7}/>
            <text x={(cx + S(t2/2) + P.webInR.x)/2 + nx*24} y={(by - S(geom.yJunc) + P.webInR.y)/2 + ny*24}
                  className="text-xs font-semibold" dominantBaseline="middle" textAnchor="middle" fill={dimHL("h3") ? "#22c55e" : "#94a3b8"}
                  transform={`rotate(${-30} ${(cx + S(t2/2) + P.webInR.x)/2 + nx*24} ${(by - S(geom.yJunc) + P.webInR.y)/2 + ny*24})`}>
              {`h3 = ${h3.toFixed(2)}`}
            </text>
          </g>
          ...
          <g stroke={dimHL("t3") ? "#22c55e" : "#94a3b8"} fill={dimHL("t3") ? "#22c55e" : "#94a3b8"} onMouseEnter={() => setHover("t3")} onMouseLeave={() => setHover(null)}>
             <line x1={ (P.juncOutR.x + P.webOutR.x)/2 + 20 } y1={ (P.juncOutR.y + P.webOutR.y)/2 } x2={ (cx + S(t2/2) + P.webInR.x)/2 } y2={ (by - S(geom.yJunc) + P.webInR.y)/2 }/>
             <text x={ (P.juncOutR.x + P.webOutR.x)/2 + 24 } y={ (P.juncOutR.y + P.webOutR.y)/2 } className="text-xs font-semibold" dominantBaseline="middle">
              {`t3 = ${t3.toFixed(2)}`}
            </text>
          </g>
          <g stroke={dimHL("t4") ? "#22c55e" : "#94a3b8"} fill={dimHL("t4") ? "#22c55e" : "#94a3b8"} onMouseEnter={() => setHover("t4")} onMouseLeave={() => setHover(null)}>
             <line x1={ (P.roofWebCornerL.x + P.apexOut.x)/2 - 20 } y1={ (P.roofWebCornerL.y + P.apexOut.y)/2 } x2={ (P.webInL.x + P.apexIn.x)/2 } y2={ (P.webInL.y + P.apexIn.y)/2 }/>
             <text x={ (P.roofWebCornerL.x + P.apexOut.x)/2 - 24 } y={ (P.roofWebCornerL.y + P.apexOut.y)/2 } textAnchor="end" className="text-xs font-semibold" dominantBaseline="middle">
               {`t4 = ${t4.toFixed(2)}`}
             </text>
          </g>

          {/* Các kích thước suy ra */}
          <Dimension x1={P.roofWebCornerR.x + 60} y1={P.apexOut.y} x2={P.roofWebCornerR.x + 60} y2={by} label={`H = ${geom.H.toFixed(2)}`} position="right" />
          <Dimension x1={P.roofWebCornerR.x + 40} y1={P.webInR.y} x2={P.roofWebCornerR.x + 40} y2={by} label={`H2 = ${geom.H2.toFixed(2)}`} position="right" />
          <Dimension x1={P.roofWebCornerR.x + 20} y1={by - S(geom.yJunc)} x2={P.roofWebCornerR.x + 20} y2={by} label={`H1 = ${geom.H1.toFixed(2)}`} position="right" />
          <Dimension x1={P.roofWebCornerL.x - 20} y1={P.apexIn.y} x2={P.roofWebCornerL.x - 20} y2={P.webInL.y} label={`H3 = ${geom.H3.toFixed(2)}`} position="left" />

          {/* La, Lb, Ld, Lc, d */}
          <Dimension x1={P.roofWebCornerL.x} y1={P.apexOut.y - 15} x2={P.roofWebCornerR.x} y2={P.apexOut.y-15} label={`La = ${geom.La.toFixed(2)}`} position="top" />
          <Dimension x1={cx} y1={P.apexOut.y} x2={P.roofWebCornerR.x} y2={P.apexOut.y} label={`Lb = ${geom.Lb.toFixed(2)}`} position="top" />
          <Dimension x1={cx} y1={P.apexIn.y - 15} x2={P.webInR.x} y2={P.apexIn.y - 15} label={`Ld = ${geom.Ld.toFixed(2)}`} position="top" />
          <Dimension x1={P.roofWebCornerL.x} y1={P.apexOut.y + 15} x2={cx} y2={P.apexOut.y + 15} label={`Lc = ${geom.Lc.toFixed(2)}`} />
          <Dimension x1={P.webOutL.x} y1={P.webOutL.y} x2={P.roofWebCornerL.x} y2={P.roofWebCornerL.y} label={`d = ${geom.d.toFixed(2)}`} position="top" />
