...
// System prompt in English; ask to answer in Vietnamese
...
export const getDesignRecommendation = async (
  ...
  if (!API_KEY) {
    return "Loi: Chua cau hinh API_KEY cua Gemini.";
  }

  const failureReasons: string[] = [];
  if (results.stress_check === 'fail') {
    failureReasons.push(`- Ung suat vuot gioi han cho phep (K_sigma = ${formatNumber(results.K_sigma, 2)} < 1).`);
  }
  if (results.deflection_check === 'fail') {
    failureReasons.push(`- Do vong vuot gioi han cho phep (n_f = ${formatNumber(results.n_f, 2)} < 1).`);
  }

  const userQuery = `Phan tich loi thiet ke dam cau truc sau day va de xuat khuyen nghi cai tien.

    Luu y: ap dung quy tac do vong cho phep [f] = L/1000 doi voi dam hop don.
    ...
    **Thong so dau vao:**
    ...
    - Do vong cho phep: [f] = L/1000 = ${results.f_allow.toFixed(3)} cm
